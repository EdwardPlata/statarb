...
public class FactorCalculator_OLD extends Calculator {
	...
	private void calculateMatrices(long asof) throws Exception {
		...
		// Writer writer = FileUtils.makeWriter("/apps/homefs1/nsarkas/rret.txt");
		// for (int row = 0; row < RRET.getRowCount(); row++) {
		// int secid = secs[row].getSecId();
		// for (int column = 0; column < RRET.getColumnCount(); column++) {
		// writer.write(secid+"|"+column+"|"+RRET.getAsDouble(row, column)+"\n");
		// }
		// }
		// writer.close();
		// System.exit(0);
